...
if ($login->isUserLoggedIn() == true) { // ************************************* logged in ******************************************************
	if($_POST['videoChatActive'] == 1) {
		if($_SESSION['user_account_login'] == 1) {
			$sql = "SELECT * FROM mentorwebdb.connections WHERE MentorID=".$_SESSION['user_id']." AND MenteeID=".$_POST['menteeID'].";";
		} elseif($_SESSION['user_account_login'] == 2) {
			$sql = "SELECT * FROM mentorwebdb.connections WHERE MentorID=".$_POST['mentorID']." AND MenteeID=".$_SESSION['user_id'].";";
		}
...
?>
<div style="height:100%; width:25%; overflow:hidden; z-index:5; position:absolute; bottom: 0px; right:0px; opacity:1; background-color:white;">
<iframe align="right" frameborder="0" height="100%" width="100%" style="" src="http://www.tlk.io/<?php echo $ConnectionName;?>" scrolling="auto" seamless></iframe>
</div>
<!-- HEADER -->
	<div id="header">
		<div class="container">
...
(function ($, window, document, undefined) {
    ...
    var settings = {
        ...
        rtcStatusUpdate: {
            ...
            "entry": {
                statusHTML: 'Please provide your full name <br>when joining chat...' + '<input id="videoChannelInput" class="video-channel-input" type="hidden" type="text" placeholder="enter name here" value="<?php echo $ConnectionName; ?>" disabled/>' + '<button id="videoChatStartButton" class="video-chat-start-button">Start Video Chat</button>',
